...
public class SimpleGraphView extends View {
     ...
     */
    private static final int[] DEFAULT_THEME = {Color.LTGRAY, Color.YELLOW, Color.GREEN, Color.GRAY, Color.BLACK,
    ...
    private final Paint mChartPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    private final Paint mTextPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    private final RectF mBoundingRect = new RectF(PADDING, PADDING, 250, 250);
    ...
    private void drawLabels(Canvas canvas) {
        if (mLabels != null && mLabels.length > 0) {
            // Draw the labels
            int x = (int) (PADDING * mDensity) + PADDING;
            int y = (int) mBoundingRect.right + PADDING;
            for (int i = 0; i < mLabels.length; i++) {
                mTextPaint.setColor(mColors[i % mColors.length]);
                canvas.drawText(mLabels[i], x, y, mTextPaint);
                x += LINE_HEIGHT;
            }
        }
    }
